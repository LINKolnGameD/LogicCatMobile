...
var Scrolling_amount: int = 13200
@onready var ButtonKeeper = $ScrollContainer/Levels
...
func _ready():
	if get_parent().get_parent().scrolling:
		Scroll.set_deferred("scroll_vertical", get_parent().get_parent().scrolling)
		Scrolling_amount = get_parent().get_parent().scrolling
	else:
#		if Scrolling_amount != 13200:
#			Scroll.set_deferred("scroll_vertical", get_parent().get_parent().scrolling)
#		else:
		Scroll.set_deferred("scroll_vertical", 13200)
##	var number = 1
...
#	Margin.add_theme_constant_override("margin_top", get_window().size.y/50)
	...
	for i in ButtonKeeper.get_children():
		if i.name == "Creative" or i.name == "Pin":
			pass
		elif successful_levels.has(i.name):
			i.disabled = false
		else:
			i.disabled = true
			if len(successful_levels) == 0:
				$ScrollContainer/Levels/Level_1.disabled = false
	...
	for i in ButtonKeeper.get_children():
		if i.name == "Creative" or i.name == "Pin":
			pass
		elif vary_successful_levels.has(i.name):
				i.icon = white_paw
		else:
			i.icon = red_paw
...
func _process(delta):
	Scrolling_amount = Scroll.scroll_vertical
	print("Scroll vert ", Scrolling_amount)
	get_parent().get_parent().scrolling = Scrolling_amount
#	var number = 0
#	for i in ButtonKeeper.get_children():
#		if menu_mood == 1:
...
func _on_creative_pressed():
	OS.shell_open("https://boosty.to/thesly/")
